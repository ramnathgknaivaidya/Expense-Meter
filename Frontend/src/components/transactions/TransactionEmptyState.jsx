...
export default function TransactionEmptyState({ onAddTransaction, hasFilters, onClearFilters }) {
  if (hasFilters) {
    return (
  ...
  return (
    <div className="empty-state">
      <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'var(--green-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
        <Plus size={28} color="var(--green)" />
      </div>
      <h3 style={{ fontSize: 18, fontWeight: 600, marginBottom: 8, color: 'var(--text)' }}>No transactions yet</h3>
      <p>Get started by adding your first transaction. Track your income and expenses to manage your finances better.</p>
      <button className="btn btn-primary" style={{ marginTop: 16, gap: 8 }} onClick={onAddTransaction}>
